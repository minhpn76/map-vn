...
export function* getCheckInLocation() {
  ...
  try {
    ...
    const typeCheckIn: string = yield select(
      makeSelectTypeCheckIn(),
    );
...
function checkInLocationRequest(typeCheckIn: string) {
  let url: string = typeCheckIn !== 'ALL' ? `api/user/checkedin_locations?type=${typeCheckIn}` : 'api/user/checkedin_locations';
