...
function apiGetDirection(
  ...
  const {map, maps, directionsRender, directionsService} = mapApi;
  const {latitude, longitude} = toLocation;
  if(map && maps){
    if (directionsRender) {
      directionsRender.setMap(null);
      directionsRender.setMap(map);
      directionsRender.setOptions({ suppressMarkers: true });
    }
          ...
          case maps.DirectionsStatus.OK:
            ...
            const infoWindow = new maps.InfoWindow({
              ...
              class="active-path" data-old_color="#000000" fill="#F39337"/> \
	            </g></g></g> </svg> <span style="color: #F39337">${langAbout} ${distanceMap}</span>`
            });
...
function clearDirection(mapApi) {
  const {directionsRender} = mapApi;
  if (directionsRender) {
    directionsRender.setMap(null);
    removeMarkers();
  }
...
function getZoomLevel(map: any, callback: (zoomLevel) => void) {
  map.addListener("zoom_changed", () => {
    callback(map.getZoom());
  });
}

export {getLocation, apiGetDirection, drawDirection, clearDirection, removeMarkers, getZoomLevel};
